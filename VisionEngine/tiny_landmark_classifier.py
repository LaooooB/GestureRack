...
MODEL_TASK = "right_hand_shape_family_v2"
...
class TinyLandmarkClassifier:
    """Pure-NumPy one-hidden-layer broad shape-family classifier.

    It intentionally predicts only OpenPalm/FoldedFour/Victory/Other. Final
    Fist-vs-Thumb and thumb direction stay in deterministic specialist geometry.
    A task marker rejects old final-gesture models instead of silently comparing
    incompatible label spaces in shadow mode.
    """
    ...
    def __init__(self, labels: Sequence[str], mean: np.ndarray, scale: np.ndarray,
                 ...
                 feature_version: int = FEATURE_VERSION, task: str = MODEL_TASK):
    ...
    def _validate(self) -> None:
        if self.feature_version != FEATURE_VERSION:
            raise ValueError(f"feature version mismatch: model={self.feature_version}, runtime={FEATURE_VERSION}")
        if self.task != MODEL_TASK:
            raise ValueError(f"model task mismatch: {self.task!r}")
        allowed = {"OpenPalm", "FoldedFour", "Victory", "Other"}
        if not self.labels or len(set(self.labels)) != len(self.labels) or set(self.labels) != allowed:
            raise ValueError("family model must contain OpenPalm/FoldedFour/Victory/Other exactly")
        ...
        if self.w2.shape != (hidden, len(self.labels)) or self.b2.shape != (len(self.labels),):
            raise ValueError("output layer must match class count")
    ...
    def load(cls, path: Path) -> "TinyLandmarkClassifier":
        with np.load(Path(path), allow_pickle=False) as data:
            ...
            task = str(np.asarray(data["task"]).reshape(-1)[0])
            return cls(
                labels=labels, mean=data["mean"], scale=data["scale"],
                w1=data["w1"], b1=data["b1"], w2=data["w2"], b2=data["b2"],
                feature_version=feature_version, task=task,
            )
    ...
    def predict_features(self, features: np.ndarray) -> TinyGesturePrediction:
        ...
        return TinyGesturePrediction(
            gesture=self.labels[top_index], confidence=confidence,
            margin=max(0.0, confidence - second),
        )

    def predict_landmarks(self, normalized_landmarks, world_landmarks=None) -> TinyGesturePrediction:
        try:
